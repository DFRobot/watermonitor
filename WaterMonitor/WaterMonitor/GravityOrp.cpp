...
void GravityOrp::update()
{
	...
	if (millis() >= orpTimer)
	{
		...
		if (orpArrayIndex == this->arrayLength) {
			orpArrayIndex = 0;
		}
		//convert the analog value to orp according the circuit
		this->orpValue = ((30 * this->voltage * 1000) - (75 * avergeArray(this->orpArray, this->arrayLength)*this->voltage * 1000 / 1024)) / 75 - this->offset;   
	}
